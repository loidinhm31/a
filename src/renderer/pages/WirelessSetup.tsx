...
export const WirelessSetup = () => {
  ...
  const discoverDevices = async () => {
    ...
    try {
      ...
      // If we found any devices, autofill the first one's IP
      if (devices.length > 0) {
        if (step === 0) {
  ...
  const parseAdbMdnsServicesOutput = (output: string): DeviceInfo[] => {
    ...
    lines.forEach(line => {
      ...
      // Example line: "adb-R58RA3M63HZ-TJb2Zx  _adb-tls-connect._tcp   192.168.1.126:39749"
      const parts = line.split(/\s+/).filter(Boolean);

      if (parts.length >= 3) {
        if (parts[1].includes("_adb-tls-connect")) {
